...
int main(int argc, char** argv) {
  ...
  if (absl::GetFlag(FLAGS_use_xds) &&
      absl::GetFlag(FLAGS_scenario_config) == "chaotic_good") {
    LOG(INFO) << "Skipping chaotic_good scenario because xDS is enabled and "
                 "the implementation is experimental/incompatible.";
    return 0;
  }
